...
import com.example.mentionview.ui.MentionsEditText;
...
public class MentionSpan extends ClickableSpan implements Parcelable {
    ...
    public void updateDrawState(@NonNull final TextPaint tp) {
        if (isSelected()) {
            tp.setColor(config.getSELECTED_TEXT_COLOR());
            tp.bgColor = config.getSELECTED_TEXT_BACKGROUND_COLOR();
        } else {
            tp.setColor(config.getNORMAL_TEXT_COLOR());
            tp.bgColor = config.getNORMAL_TEXT_BACKGROUND_COLOR();
        }
    ...
    public void setDisplayMode(@NonNull Mentionable.MentionDisplayMode mode) {
        mDisplayMode = mode;
    }
    ...
    public void writeToParcel(@NonNull final Parcel dest, int flags) {
        dest.writeInt(config.getNORMAL_TEXT_COLOR());
        dest.writeInt(config.getNORMAL_TEXT_BACKGROUND_COLOR());
        dest.writeInt(config.getSELECTED_TEXT_COLOR());
        dest.writeInt(config.getSELECTED_TEXT_BACKGROUND_COLOR());
